...
const transform: AxiosTransform = {
  ...
  transformRequestData: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    if (code === ResultEnum.SUCCESS) {
      // auth登录超时处理
      if (message && message.includes('D1600')) {
        // 到登录页
        errorMsg = '登录超时，请重新登录!'
        $dialog.warning({
          title: '提示',
...
export const httpFile = createAxios({
  requestOptions: {
...
export const httpAuth = createAxios({
  requestOptions: {
    apiUrl: import.meta.env.VITE_AUTH_API
  }
})
